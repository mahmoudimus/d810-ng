...
version: 2025-11-06
related_skills:
  - ci-debugging (specific CI workflow debugging)
...
**Environment:** Requires `GITHUB_TOKEN` environment variable. If does not exist, prompt for one and one will provided for you.
...
if command -v gh &> /dev/null; then
  GH="gh"
elif [ -f "/tmp/gh-install/gh_2.83.1_linux_amd64/bin/gh" ]; then
...
- **"command not found"** → Use full path `/tmp/gh-install/gh_2.83.1_linux_amd64/bin/gh`
...
ci-debugging, project-verification, project-workflow, using-superpowers
