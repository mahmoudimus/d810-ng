...
- `d810ng-d7j`: Hybrid Canonical pattern matching → P4 (Shadow Map problem too complex)
...
## Session Part 2: SDK pxd Equivalence Testing

### What Was Done

1. **Verified SDK pxd files compile** - All files in `src/d810/speedups/cythxr/sdk/` pass Cython syntax check with 0 errors

2. **Compared mop_t structure** - SDK hexrays.pxd has ALL union members:
   - `r` (mreg_t), `nnn` (mnumber_t*), `d` (minsn_t*), `s` (stkvar_ref_t*)
   - `g` (ea_t), `b` (int), `f` (mcallinfo_t*), `l` (lvar_ref_t*)
   - `a` (mop_addr_t*), `helper` (char*), `cstr` (char*), `c` (mcases_t*)
   - `fpc` (fnumber_t*), `pair` (mop_pair_t*), `scif` (scif_t*)

3. **Compared minsn_t structure** - All fields match:
   - `opcode`, `iprops`, `next`, `prev`, `ea`, `l`, `r`, `d`
   - Methods: `dstr()`, `setaddr()`, `optimize_solo()`, `is_helper()`, etc.

4. **Proved equivalence with real code** - Decompiled `_test_chained_add` and accessed:
   ```
   minsn_t.l: type=mop_l, mop.l.idx = 0       (lvar_ref_t*)
   minsn_t.r: type=mop_r, mop.r = 248         (mreg_t)
   minsn_t.d: type=mop_d, mop.d.opcode = 13   (minsn_t*)
   ```

5. **Identified gaps for migration** - SDK pxd needs custom helpers:
   - `_swig_ptr` - Extract C++ pointer from SWIG
   - `MOPT` enum - Mop type constants
   - `OPERAND_PROPERTIES` enum
   - `qstring` typedef
   - `get_mreg_name` function

### Issue Created
- `d810ng-sm4`: Migrate to SDK-generated pxd files for Cython bindings

### Key Finding
SDK pxd files are **structurally equivalent** and have MORE methods than manual version. Migration is feasible with addition of custom helpers.
