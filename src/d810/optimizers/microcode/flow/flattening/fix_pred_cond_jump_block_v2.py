"""Optimized FixPredecessorOfConditionalJumpBlock with caching and fast data structures.

This is an optimized version of fix_pred_cond_jump_block.py based on profiling results:

Performance improvements:
1. Cache predecessor analysis results - avoids re-tracing same paths
2. Use optimized tracker components - ImmutableBlockInfo, CachedBlockPath
3. Lazy logging - avoid formatting strings when logging disabled
4. Batch predecessor analysis - analyze all at once

Original profiling (8.3s for hodur_func):
- block_serial_path: 1.32s (102K calls) -> Cached: target <0.1s
- MopHistory.get_copy: 0.76s (22K calls) -> Structural sharing: target <0.2s
- search_backward: 113x recursive amplification -> Caching: target 10x reduction

Target: <1s for hodur_func (8x+ speedup)
"""
...
class PredecessorAnalysisCache:
    ...
    repeatedly across passes. Caching the results provides huge speedup.

    Cache key: (mba_id, blk_serial, pred_serial, state_var_hash, maturity)
    ...
    def _make_key(
        ...
        """Create cache key."""
        # Use mba.entry_ea as a stable identifier for the function
...
# Global cache instance (shared across passes)
_pred_analysis_cache = PredecessorAnalysisCache()


class FixPredecessorOfConditionalJumpBlockV2(GenericUnflatteningRule):
    """Optimized version with caching and fast data structures.
    ...
    DESCRIPTION = "Optimized: Detect if a predecessor of a conditional block always takes the same path and patch it"
    ...
    def _get_state_var_repr(self, blk: ida_hexrays.mblock_t) -> str:
        """Get a stable string representation of the state variable being compared.

        Cached to avoid repeated formatting.
        """
        if blk.serial in self._state_var_repr_cache:
            return self._state_var_repr_cache[blk.serial]

        # Format the left operand of the jump instruction
    ...
    def is_jump_taken(
        ...
        # Signed comparisons - need to convert values
        def to_signed(v):
    ...
    def _analyze_predecessor(
        ...
        if cached is not None:
            # Cached result - check if it contains None (unknown)
        ...
        # Log if enabled (lazy formatting)
    ...
    def sort_predecessors(self, blk: ida_hexrays.mblock_t):
        ...
        This is the main optimization target - uses caching and lazy logging.
        """
        ...
        for pred_serial in blk_preset_list:
            pred_values, has_unknown = self._analyze_predecessor(
                blk, pred_serial, op_compared, state_var_repr
            )
            ...
            if is_jmp_always_taken and is_jmp_never_taken:
                ...
                if unflat_logger.isEnabledFor(40):  # ERROR level
                    unflat_logger.error(
                        "Logic error: jump both always and never taken from %d: %s",
                        pred_serial, pred_values
                    )
                pred_jmp_unk.append(pred_blk)
                continue

            if is_jmp_always_taken:
                if unflat_logger.isEnabledFor(20):
                    unflat_logger.info(
                        "Jump always taken from pred %d: %s",
                        pred_serial, pred_values
                    )
            ...
            if is_jmp_never_taken:
                if unflat_logger.isEnabledFor(20):
                    unflat_logger.info(
                        "Jump never taken from pred %d: %s",
                        pred_serial, pred_values
                    )
    ...
    def analyze_blk(self, blk: ida_hexrays.mblock_t) -> int:
        ...
        if unflat_logger.isEnabledFor(20):
            unflat_logger.info(
                "Block %d has %d preds: %d always, %d never, %d unk",
    ...
    def optimize(self, blk: ida_hexrays.mblock_t) -> int:
        ...
        if self.last_pass_nb_patch_done > 0:
            ...
            safe_verify(
                self.mba,
                "optimizing FixPredecessorOfConditionalJumpBlockV2",
