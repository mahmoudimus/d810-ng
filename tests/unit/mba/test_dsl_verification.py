...
4. VerifiableRule subclass definition (without IDA)
"""
...
class TestVerifiableRuleDefinition:
    """Tests for defining VerifiableRule subclasses without IDA."""

    def test_can_define_rule_subclass(self):
        """Test that VerifiableRule subclass can be defined without IDA.

        NOTE: A warning will be logged about registration failure, but
        the class definition should succeed.
        """
        from d810.mba.dsl import Var
        ...
        class TestXorRule(VerifiableRule):
            PATTERN = (x | y) - (x & y)
            REPLACEMENT = x ^ y

        # The DSL patterns should be captured
        assert hasattr(TestXorRule, "_dsl_pattern")
        assert hasattr(TestXorRule, "_dsl_replacement")
        assert str(TestXorRule._dsl_pattern) == "((x | y) - (x & y))"
        assert str(TestXorRule._dsl_replacement) == "(x ^ y)"

    def test_can_verify_rule_without_instantiation(self):
        """Test Z3 verification using DSL patterns directly.

        This bypasses the instantiation which tries to access .node
        and demonstrates pure verification works.
        """
        from d810.mba.dsl import Var
        from d810.mba.rules import VerifiableRule
        from d810.mba.backends.z3 import prove_equivalence
        ...
        class TestMbaRule(VerifiableRule):
            DESCRIPTION = "MBA XOR simplification"
            PATTERN = x + y - Var("2") * (x & y)  # Using Var for 2 as placeholder
            REPLACEMENT = x ^ y

        # Actually let's use concrete constants
        from d810.mba.dsl import Const

        TWO = Const("2", 2)

        class TestMbaRule2(VerifiableRule):
            DESCRIPTION = "MBA XOR simplification with constant"
            PATTERN = x + y - TWO * (x & y)
            REPLACEMENT = x ^ y

        # Verify directly with prove_equivalence
        is_equiv, counter = prove_equivalence(
            TestMbaRule2._dsl_pattern, TestMbaRule2._dsl_replacement
        )
        assert is_equiv is True, f"Counterexample: {counter}"
